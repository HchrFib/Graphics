...
void Multitex::onPluginLoad()
{
    ...
    // parametro1 : target
    // parametro2 : para este target nivel de detalle 0. lod0  NO USAMOS MIPPING
...
void Multitex::preFrame() 
{
    ...
    // bind textures (aqui hacemos que se activen las texturas)
    g.glActiveTexture(GL_TEXTURE0);
    g.glBindTexture(GL_TEXTURE_2D, textureId0); // sampler0
    g.glActiveTexture(GL_TEXTURE1);
    g.glBindTexture(GL_TEXTURE_2D, textureId1); // sampler1
}
