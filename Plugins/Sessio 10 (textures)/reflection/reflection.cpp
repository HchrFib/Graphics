...
void drawRect(GLWidget &g, Point V0, Point V1, Point V2, Point V3)
{
    ...
    float coords[] = { 1.3*V0.x(), V0.y(), 1.3*V0.z(),
                       1.3*V1.x(), V1.y(), 1.3*V1.z(),
					   1.3*V3.x(), V3.y(), 1.3*V3.z(),
					   1.3*V2.x(), V2.y(), 1.3*V2.z() };
...
bool Reflection::paintGL()
{
	...
	reflection1.scale(1,-1,1);
    reflection1.translate( 0,-2*scene()->boundingBox().min().y(), 0);

	g.defaultProgram()->setUniformValue("modelViewProjectionMatrix", 	
		camera()->projectionMatrix() * camera()->viewMatrix() * reflection1);
	if (drawPlugin()) drawPlugin()->drawScene();

	// Get texture for bottom plane (generamos la textura con la copa mirando hacia abajo)
    g.glBindTexture(GL_TEXTURE_2D, textureId);		
    ...
    V2 = Point(b.max().x(), b.min().y(), b.max().z());
    V3 = Point(b.min().x(), b.min().y(), b.max().z());
